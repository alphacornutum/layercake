...
4. **Eval** — `ae_eval_script` pre-validates agent ExtendScript (ES3 dialect / trailing-comma strip in `extendscript-compat.ts`), wraps with JSON polyfill + result-file protocol, runs via AppleScript `DoScriptFile` (macOS) or `AfterFX.exe -r` (Windows), parses `OK`/`ERR`.
...
| `extendscript-execution`       | `ae_eval_script`                                                               | `src/host/extendscript-compat.ts`, `script-wrapper.ts`, `macos.ts`, `windows.ts`                    |
...
- **ES3-compatible ExtendScript at runtime** — author modern TS under `src/ae-scripts/`; `build:ae-scripts` emits ES5/`var` payloads AE can run; `ae_eval_script` pre-validates agent source as ES3 (`extendscript-compat.ts`); `JSON` comes from the injected polyfill. Dialect guidance: `skills/drive-after-effects/references/extendscript.md`.
