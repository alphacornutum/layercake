// @ts-nocheck
import { isRootFolder, itemById, itemTypeName, parentFolderInfo } from "../shared/inventory";
import { resolveComp, resolveLayer } from "../shared/resolve";
import {
  applyStyleToDoc,
  projectTextDocument,
  styleSnapshotFromProjection,
  suppliedStyleMatches,
} from "../shared/text-document";
import {
  applyCreateSolid,
  applyDeleteLayer,
  applyRenameProjectItem,
  applyReplaceLayerSource,
  applyResetLayerSurface,
  applySafeDeleteProjectItem,
  applySetCompSettings,
  applySetLayerIndex,
  applySetLayerSwitches,
  applySetLayerTiming,
  applySetLayerTransform,
  applySetPropertyExpression,
} from "../shared/control-plane";

declare const payload: any;
declare const MAX_TARGETS: number;
declare const UNDO_NAME: string;

function resolveFail(code: string, message: string, candidates: object[] | null): never {
  var errorPayload = { code: code, message: message };
  if (candidates) errorPayload.candidates = candidates;
  throw new Error("AFX_RESOLVE:" + JSON.stringify(errorPayload));
}

// Legacy helper surface retained verbatim while individual helpers are typed incrementally.
// @ts-nocheck
...
function resolveLayerTarget(ref) {
  var comp = resolveComp(ref.compId, ref.compName, resolveFail);
  var layer = resolveLayer(comp, ref.layerId, ref.layerName, resolveFail);
...
function resolveTextSelector(selector) {
  ...
  if (kind === "layers") {
    ...
    for (var ci = 0; ci < compIds.length; ci++) {
      var byId;
      try {
        byId = resolveComp(compIds[ci], null, resolveFail);
      } catch (ce) {
    ...
    for (var cn = 0; cn < compNames.length; cn++) {
      var byName;
      try {
        byName = resolveComp(null, compNames[cn], resolveFail);
      } catch (cne) {
...
function readAuthoredTextDocument(textProp, comp) {
  try {
    if (
      textProp.canVaryOverTime ||
      (textProp.expression && String(textProp.expression).length > 0)
    ) {
...
function resolveOp(op) {
  ...
  if (op.op === "rename_project_item") {
    ...
    return {
      plan: {
        op: op,
        kind: "rename_project_item",
        item: renameItem,
        targets: [{ item: renameItem }],
      },
      targetCount: 1,
    };
  }
  ...
  if (op.op === "set_layer_switches") {
    var switchesPair;
    try {
      switchesPair = resolveLayerTarget(op.target);
    } catch (swre) {
      return { error: formatResolveError(swre) };
    }
    return {
      plan: { op: op, kind: "set_layer_switches", targets: [switchesPair] },
      targetCount: 1,
    };
  }
  if (op.op === "set_comp_settings") {
    var settingsComp;
    try {
      settingsComp = resolveComp(op.target.compId, op.target.compName, resolveFail);
    } catch (csre) {
