// @ts-nocheck
/** TextDocument projection and mutation helpers, bundled into patch-apply. */
...
function justificationToString(j) {
  try {
    ...
    if (j === ParagraphJustification.FULL_JUSTIFY_LASTLINE_LEFT)
      return "FULL_JUSTIFY_LASTLINE_LEFT";
    if (j === ParagraphJustification.FULL_JUSTIFY_LASTLINE_RIGHT)
      return "FULL_JUSTIFY_LASTLINE_RIGHT";
    if (j === ParagraphJustification.FULL_JUSTIFY_LASTLINE_CENTER)
      return "FULL_JUSTIFY_LASTLINE_CENTER";
    if (j === ParagraphJustification.FULL_JUSTIFY_LASTLINE_FULL)
...
function justificationFromString(s) {
  ...
  if (s === "FULL_JUSTIFY_LASTLINE_RIGHT")
    return ParagraphJustification.FULL_JUSTIFY_LASTLINE_RIGHT;
  if (s === "FULL_JUSTIFY_LASTLINE_CENTER")
...
export function styleSnapshotFromProjection(proj) {
...
export function projectTextDocument(doc) {
  if (!doc) return null;
  var style = {};
  var keys = [];
  ...
  try {
    out.boxText = !!doc.boxText;
  } catch (e1) {}
  try {
    out.pointText = !!doc.pointText;
  } catch (e2) {}
  return out;
}
...
export function suppliedStyleMatches(styleSnap, supplied) {
 ...
 */
export function applyStyleToDoc(doc, style, allStyleRuns) {
  ...
  if (preservedCaps) {
    try {
      if (typeof FontCapsOption === "undefined") {
        return "fontCapsOption requires After Effects 24+ (LayerCake supports AE 24.6+)";
      }
