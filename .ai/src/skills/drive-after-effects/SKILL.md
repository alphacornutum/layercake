...
8. **Typed patch** — Prefer `ae_patch_project` (apply-only) over raw eval for routine `set_text_style` font fixes and Project panel `create_folder` / `move_project_item` / `delete_project_item`. Pass `project.path` + `project.fingerprint` guards. On success, reuse the returned fingerprint for the next save/patch when no other mutator (human UI, `ae_eval_script`, another tool) ran; otherwise re-bind context. Delete follows AE defaults (recursive folder remove; in-use items may be deleted) — read `nestedItemCount` and full `usedInCompIds` in evidence. Patch mutates authored / pre-expression state; panel ops do not rewrite expression strings.
...
- Prefer `ae_patch_project` for typed text-style and panel create/move/delete; leave persistence to `ae_save_project`.
