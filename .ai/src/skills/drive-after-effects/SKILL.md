...
description: >-
  ...
  "normalize fonts to Arial", "create a solid", "create text", "convert to box text",
  "point text", "paragraph text", "replace layer source", "safe delete footage",
...
- `ae_eval_script` can mutate the open project and bypasses patch guards — say so when running destructive scripts. Write ExtendScript (ES3); modern JS is refused before the host — see `references/extendscript.md`. Empty MCP success with a dirty project usually means a missing top-level `return` (e.g. bare IIFE), not truncation.
...
- **`set_property_expression` effect paths:** `ae_get_layer` shows a nested tree, not a flat matchNames path. Effect **parameters** (e.g. Slider Control slider `ADBE Slider Control-0001`) are not direct children of `ADBE Effect Parade` — include the **effect instance** segment: `["ADBE Effect Parade", "ADBE Slider Control", "ADBE Slider Control-0001"]` when one Slider Control exists, or `["ADBE Effect Parade", "<effect display name>", "ADBE Slider Control-0001"]` when multiple share the matchName. Skipping the middle segment fails. Prefer typed patch over eval for routine effect slider expressions.
- Prefer `create_text` over eval for new horizontal point or box text layers. Typed create ops (`create_folder`, `create_solid`, `create_text`) may omit `name` — host/AE default; evidence returns the final name. Explicit names remain useful for panel structure.
- **Point vs box text:** `boxText` / `pointText` are read-only in AE — there is no typed in-place convert. Detect via `ae_get_layer` (`extended`/`full`) SourceText projection. To change layout: `create_text` with the other `layout` (point→box needs an intentional `boxTextSize`), copy transform/timing/switches/name/index with typed ops as needed, then `delete_layer` the old layer. The new layer has a **new `Layer.id`**; effects, masks, parenting, expressions, and SourceText keyframes are not transferred by this recipe.
