...
Contributor note: the **canonical end-user** skill lives at `skills/drive-after-effects/`. Keep this file aligned when editing workflow guidance.
...
5. **Inventory as needed** — Prefer `ae_list_comps`, `ae_list_sources`, and/or `ae_list_folders` before writing custom scripts. Panel ops need real `Item.id` handles from inventory (including `rootFolder.id` — never a magic `0`).
...
8. **Typed patch** — Prefer `ae_patch_project` (apply-only) over raw eval for routine `set_text_style` font fixes, layer renames (`rename_layer`), and Project panel `create_folder` / `move_project_item` / `delete_project_item`. Pass `project.path` + `project.fingerprint` guards. Layer targets (`rename_layer`, `set_text_style` layer/comp selectors) accept id or unique name with the same ambiguity rules as `ae_get_layer` — prefer stable ids when names may collide; panel item ops stay on `Item.id`. Successful targets include post-condition-verified before/after evidence. For `set_text_style`, post-condition uses authored `fonts` (pre-expression); evidence may also include `evaluatedFonts` (post-expression / on-screen). When authored matches but evaluated still differs, patch expression source layers (for example a `{font}` controller) before or with consumers so on-screen results update. On success, reuse the returned fingerprint for the next save/patch when no other mutator (human UI, `ae_eval_script`, another tool) ran; otherwise re-bind context. Delete follows AE defaults (recursive folder remove; in-use items may be deleted) — read `nestedItemCount` and full `usedInCompIds` in evidence. Patch mutates authored / pre-expression state; panel ops do not rewrite expression strings.
...
- Prefer `ae_patch_project` for typed text-style, `rename_layer`, and panel create/move/delete; leave persistence to `ae_save_project`. Prefer ids over names when targeting layers that may share a name.
- Folder delete can recursively remove contents; deleting in-use footage/comps breaks layers — check impact evidence first.
