...
- **`set_layer_timing` does not move keyframes** (composition-absolute key times stay put). For **source slip** (same parent in/out, different nested source range — e.g. show nested 0–90 instead of 10–100 in the same window), send new `startFrame` **plus** unchanged `inFrame`/`outFrame` in one op — not `startFrame` alone (AE may nudge the trim). Assumes remapping is off.
