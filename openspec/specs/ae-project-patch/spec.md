...
Declarative apply-only mutation tool with typed ops (text style, rename layer/item, panel create/move/delete, and control-plane mutators including solids, source replace, frame timing, expressions, layer reset/delete, and `safe_delete_project_item`), fingerprint guards, undo grouping, and post-condition-verified before/after evidence.
